    ...
    for (size_type i{}; i < n_cells_x; ++i) {
        ...
        for (size_type j{}; j < n_cells_y; ++j) {
            for (size_type k{}; k < n_cells_z; ++k) {
                ...
                n_particles += static_cast<value_type>(flow_properties_->n_particles[c]) / n_cells_yz;
        ...
        file << x << '\t' << n_particles << '\t' << n_density << '\t' << ux << '\t' << uy << '\t' << uz << '\t' << u2x << '\t'
             << u2y << '\t' << u2z << '\t' << Ttrx << '\t' << Ttry << '\t' << Ttrz << '\t' << Ttr << '\n';
    }
